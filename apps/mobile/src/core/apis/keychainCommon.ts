...
import { appEncryptor } from '../services/keyringRuntime';
import { getRegisteredDeferredService } from '../services/deferred';
import { callLockDeferredService } from '../services/lockDeferredClient';
import {
  PREFERENCE_DEFERRED_SERVICE,
  type PreferenceDeferredService,
} from '../services/preferenceDeferred';
import {
...
function isPasswordAutoGenerated() {
  return (
    getRegisteredDeferredService<PreferenceDeferredService>(
      PREFERENCE_DEFERRED_SERVICE,
    )?.getPasswordIsAutoGenerated() || false
  );
}
...
export function createBusinessKeychainApi({
      ...
      if (keychainObject.password) {
        ...
        switch (purpose) {
          case RequestGenericPurpose.VERIFY: {
            const verifyResult = await callLockDeferredService(
          ...
          case RequestGenericPurpose.DECRYPT_PWD: {
            ...
            await callLockDeferredService('updateUnlockTime', []);
            await runPostDecryptKeychainRewrite();
            onRequestReturn(instance);
  ...
  async function clearApplicationPassword(password: string) {
    return Promise.allSettled([
      callLockDeferredService('clearCustomPassword', [password]),
      resetGenericPassword(),
